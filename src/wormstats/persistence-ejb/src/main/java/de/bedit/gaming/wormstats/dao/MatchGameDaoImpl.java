...
public class MatchGameDaoImpl implements MatchGameDao {
    ...
    private static final Logger logger = Logger
            .getLogger(MatchGameDaoImpl.class.getName());

    @Override
    public void createMatchGame(MatchGame matchGame) {
        logger.log(Level.INFO, "Create new matchGame with id {0}",
    ...
    public void updateMatchGame(MatchGame matchGame) {
        logger.log(Level.INFO, "Update matchGame with id {0}",
                new Object[] { matchGame.getId() });
        em.merge(matchGame);
    }
    ...
    public void deleteMatchGame(MatchGame matchGame) {
        logger.log(Level.INFO, "Delete new matchGame with id {0}",
                new Object[] { matchGame.getId() });
        em.merge(matchGame);
        em.remove(matchGame);
    }
    ...
    public List<MatchGame> getAllMatchGamesByWinnerId(long id) {
        return em.createNamedQuery("getAllMatchGamesByWinnerId").setParameter(
                "id", id).getResultList();
    }
}
